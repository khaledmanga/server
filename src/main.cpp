...
int main() {
  ...
  router.add("/hello", [](Request &request) {
    Response response;
  ...
  while (true) {
    ...
    std::string current_path = request.path();

    const auto *matched_pair = router.match(current_path);
    ...
    if (matched_pair == nullptr) {
      ...
      send(client_fd, serialized_data.c_str(), serialized_data.size(),
           MSG_NOSIGNAL);
