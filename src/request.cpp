...
const std::string &RequestLine::method() const noexcept { return method_; }

const std::string &RequestLine::path() const noexcept { return path_; }

uint8_t RequestLine::majorVersion() const noexcept { return major_version_; }

uint8_t RequestLine::minorVersion() const noexcept { return minor_version_; }
...
const std::string &Header::host() const noexcept { return host_; }

const std::string &Header::userAgent() const noexcept { return user_agent_; }

const std::string &Header::accept() const noexcept { return accept_; }

const std::string &Header::contentType() const noexcept {
  return content_type_;
}

int Header::contentLength() const noexcept { return content_length_; }
...
const Header &Request::header() const noexcept { return header_; }

const std::string &Request::body() const noexcept { return body_; }
...
void Request::parseRequestLine(const std::string &request_line_raw) {
  ...
  setVersion(static_cast<uint8_t>(std::stoi(
                 version.substr(pos_slash + 1, pos_dot - pos_slash - 1))),
...
void Request::printRequestLine() const {
  ...
  std::cout << "Version: HTTP/" << static_cast<int>(majorVersion()) << "."
...
void Request::printBody() const { std::cout << "Body: " << body_ << std::endl; }